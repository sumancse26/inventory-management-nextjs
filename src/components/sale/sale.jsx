...
const Sale = () => {
    ...
    const searchProduct = (e) => {
        ...
        if (searchKey || searchKey == '') {
            const searchedProduct = productList?.filter((item) =>
                Object.values(item).some((value) => String(value).toLowerCase().includes(searchKey?.toLowerCase()))
            );
    ...
    const subTotal = () => {
        return addedProduct.reduce((total, item) => total + Number(item.item_total || 0), 0);
    };
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-4 h-[calc(100vh-150px)]">
                        ...
                        <div className="col-span-12 md:col-span-8 bg-white shadow-xl rounded-2xl p-6 animate-fadeIn flex flex-col">
                            <div className="flex justify-between items-start border-b border-gray-300 pb-4">
                                <div>
                                    <h2 className="text-lg font-bold text-gray-700 mb-2">Billed To</h2>
                                    <p className="text-sm text-gray-600">
                                        Name:
                                        <span className="text-indigo-700">{selectedCustomer.name || ''}</span>
                                    </p>
                                    <p className="text-sm text-gray-600">
                                        Email:
                                        <span className="text-indigo-700">{selectedCustomer.email || ''}</span>
                                    </p>
                                    <p className="text-sm text-gray-600">
                                        Phone:
                                        <span className="text-indigo-700">{selectedCustomer.mobile || ''}</span>
                                    </p>
                                </div>
